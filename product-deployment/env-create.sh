...
deployment_location=$(grep -E "^[ ]*location[ ]+=" "$main_conf_file" | awk '{$1=$2=""; gsub(/^[ \t]+|[ \t]+$/,"",$0); print $0}' | tr -d '"')
...
echo -e "Location       : ${BOLD}${deployment_location}${DEFAULT}"
echo
...
# =====================================================================================================================
   print_header "Creating Infrastructure for Security ${deployment_environment}"
   echo
